import { GetHomepage } from '../queries/pageQueries/homeQuery'
import { allBlocks } from '@/queries/fragments/blocks'
...
import { allWorksSlugs, workSingle } from '@/queries/pageQueries/workQuery'
...
export async function getAllWorksWithSlug() {
  ...
  return entries?.data?.workCollection?.items
}

export async function getWork(slug, preview) {
  const entry = await fetchGraphQL(
      workSingle(slug, preview),
      preview
  )
  return {
      work: entry?.data?.workCollection?.items,
  }
}
